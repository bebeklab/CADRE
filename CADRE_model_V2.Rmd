---
title: "Prediction for melanoma with CADRE model"
...
requiredPackages2=c("Biobase","dplyr","fgsea","ggplot2","igraph","limma","parallel","pROC","PRROC","tidyr")
...
# example here used the 9st row of sheet, melanoma.
...
thresholds=c(0:floor(max_of_row_mins),Inf)
share_synergy_list=list()
share_antagonism_list=list()


score_sheet <- data.frame(synergy_count = (length(thresholds) - 1), antagonism_count = (length(thresholds) - 1))
...
for (i in 1:(nrow(compoundnames_unique) - 1)) {
  for (j in (i + 1):nrow(compoundnames_unique)) {
    ...
    if(num_cellline!=0){
      temp_sum=NULL
      for(h in 1:(length(thresholds) - 1)){
...
# this step takes a lot of time, about 10 mins or longer depends on computer's performance
...
rownames(drug_pathway_df)=pathway_list[,1]
write.csv(drug_pathway_df,paste0(paste0(tissue_folder,"csv","/drug_pathway_",tissue,"_kegg.csv")))
```
...
weight_drug_pathway_df <- full_join(property_data1, property_data1, c('property' = 'property')) %>% 
...
```{r, error = TRUE}
graph_pathway <- graph_from_data_frame(weight_drug_pathway_df, directed = FALSE)
...
print(paste0(" AUROC for tissue ",tissue, " = ",auc(roc_obj)))

pr_curve <- pr.curve(scores.class0 = prediction1$score, weights.class0 = prediction1$probability, curve = TRUE)
print(paste0(" AUPRC for tissue ",tissue, " = ",pr_curve$auc.integral))


write.table(data.frame(c("AUROC",auc(roc_obj)),c("AUPRC",pr_curve$auc.integral)), file = paste0(tissue_folder,tissue,"_auc.txt"), row.names = FALSE, col.names = FALSE, quote = FALSE)
```
